...
- `iostream`头文件：从标准流中读写数据，`istream`从流中读取数据、`ostream`从流中写入数据。
- `fstream`头文件：从文件中读写数据，`ifstream`从文件中读取数据、`ofstream`从文件中写数据。
- `sstream`头文件：从字符串中读写数据，`istringstream`从string中读取数据、`ostringstream`从string中写数据

### IO对象不可复制或赋值

进行IO操作的函数通常以引用的方式传递和返回流。读写一个IO对象会改变其状态，因此传递和返回的引用不能是const的。
...
- **每个输出流都管理一个缓冲区，执行输出的代码，文本串可能立即打印出来，也可能被操作系统保存在缓冲区内，随后再打印，这就涉及缓冲区的刷新程度**。
- 缓冲区刷新的方式
  - 程序正常结束的时候
  - 缓冲区满的时候，需要刷新缓冲
  - 使用操控符
...
操作符基本上与我们之前已经使用过的对象cin和cout一样。特别是，我们也可以使用`>>`,`<<`来读写文件，也可以使用`getline`来从一个流中读取数据。处理继承iostream的上述那些特性之外，文件流还有下述的操作。
...
因为fstream使继承于iostream，所以可以接受iostream类型引用参数的函数，可以用一个对应的fstream类型来调用。
...
一般默认以ifstream关联的文件默认以in的方式打开，以ofstream关联的文件默认以out模式打开。以out打开的时候，会默认将文件内容清空，如果不想，需要同时指定追加模式。`osftream out("file",ofstream::out|ofstream::app)`
...
```cpp
void readFile(const string filepath,vector<string>& vec)
{
